...
import { Shield, Mail, Lock, Zap, BarChart3 } from "lucide-react";
...
const Login = () => {
    ...
    const submitHandler = async (e) => {
        e.preventDefault();
        try {
            const res = await axios.post(`${apiUrl}/api/user/login`, input, {
                headers: {
    ...
    return (
        <div className="w-screen h-screen flex flex-col justify-center items-center bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 relative overflow-hidden">
            ...
            {Array.from({ length: 15 }).map((_, i) => (
                <div
                    key={i}
                    className="absolute text-xs font-mono opacity-20 pointer-events-none text-cyan-400"
                    style={{
            ...
            >
                ...
                <div className="w-full flex flex-col items-center mb-4">
                    ...
                    <div className="w-16 h-16 bg-gradient-to-r from-cyan-400/20 to-purple-500/20 rounded-full flex items-center justify-center mb-4 border border-purple-500/30">
                        <Shield className="text-3xl text-purple-400" />
                    </div>
                ...
                <button
                    type="submit"
                    className="group relative w-full py-4 bg-gradient-to-r from-cyan-500 to-purple-600 rounded-xl font-semibold text-white shadow-lg hover:scale-105 transition-all duration-300 hover:shadow-cyan-500/25"
                >
